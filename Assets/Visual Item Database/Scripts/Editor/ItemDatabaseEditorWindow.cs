...
public partial class ItemDatabaseEditorWindow : EditorWindow
{
    ...
    static readonly string databasePath = @"Assets/Visual Item Database/Resources/" + ItemDatabaseV32.dbName + ".asset";   //Path of the item database
    readonly string itemPrefabsPath = @"Assets/Visual Item Database/Prefabs/Item Prefabs/"; //Path that contains the category folders
    static readonly string vidListsPath = @"Assets/Visual Item Database/Resources/" + VIDItemListsV32.itemListsName + ".asset"; //Path that contains the category folders
    readonly string vidListsScriptPath = @"Assets/Visual Item Database/Scripts/" + VIDItemListsV32.itemListsName + ".cs"; //Path that contains the category folders
    ...
    public static void CreateItemDatabase()
    {
        ...
        if (!AssetDatabase.LoadAssetAtPath(vidListsPath, typeof(VIDItemListsV32)))
        {
            AssetDatabase.CreateAsset(CreateInstance<VIDItemListsV32>(), vidListsPath);
            AssetDatabase.DeleteAsset(databasePath);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();

            Debug.Log("'VIDItemListsV31' asset created in Resources folder");
        }

        //If an ItemDatabse doesn't exists then create one(We don't use the generic function to support older unity versions)
        if (!AssetDatabase.LoadAssetAtPath(databasePath, typeof(ItemDatabaseV32)))
        {
            //Clear those keys
            EditorPrefs.DeleteKey("ItemToShowIndex");
            EditorPrefs.DeleteKey("ListIndex");

            ScriptableObject database = CreateInstance<ItemDatabaseV32>();
            AssetDatabase.CreateAsset(database, databasePath);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();

            Debug.Log("'" + ItemDatabaseV32.dbName + "' created in Resources folder");
        }
    }
    ...
    public static ItemDatabaseEditorWindow OpenDatabaseWindow()
    {
        ...
        if (!AssetDatabase.LoadAssetAtPath(databasePath, typeof(ItemDatabaseV32)) || !AssetDatabase.LoadAssetAtPath(vidListsPath, typeof(VIDItemListsV32)))
            CreateItemDatabase();
    ...
    {
        //Load database
        autoVidLists = (VIDItemListsV32)AssetDatabase.LoadAssetAtPath(vidListsPath, typeof(VIDItemListsV32));
        database = (ItemDatabaseV32)AssetDatabase.LoadAssetAtPath(databasePath, typeof(ItemDatabaseV32));
    ...
    void MiniSideButtonsControl(int index)
    {
        ...
        if (GUILayout.Button(new GUIContent("-", "Delete Item"), EditorStyles.miniButtonLeft, GUILayout.Width(20)))
        {
            ...
            if (!confirmBeforeDeleting || ItemRemovalConfirmed())
            {
                ...
                if (selectedSubtype > 0)
                {
                    ...
                    if (autoVidLists.subtypes[GetSubtypeIndex(subtypeNames[selectedSubtype])].itemIDs.Count - 1 == index)
                        itemToShow.itemToShowIndex = -1;

                    //Get item index in the main type list
        ...
        else if (GUILayout.Button(new GUIContent("+", "Duplicate Item"), EditorStyles.miniButtonRight, GUILayout.Width(20)))
        {
            ...
            {
				case ItemType.Item:
					autoVidLists.autoItem.Insert(index, (Item)itemCont.item);
					break;
            }//#VID-2MSB
