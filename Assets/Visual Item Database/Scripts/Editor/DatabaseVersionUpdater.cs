...
public partial class DatabaseVersionUpdater : EditorWindow
{
    ...
    static readonly string partialClassName = "DatabaseVersionUpdater32";
    static readonly string currentDir = CorrectPathSeparators(Directory.GetCurrentDirectory()) + "/";
    string tempSettingsFullPath;

    bool itemTypeWindowisGood = false, updateNamesOnExit;
    ...
    public static void IsUpdatingDatabase()
    {
        ...
        if (EditorUtility.DisplayDialog("Are You Updating?", "If You have Just Downloaded a New Version Of The Visual Item Database and You Have an Old Database With Items In It, " +
            "Then Please Press 'Update' So we Can Move Your Items to The New Version.", "Update", "I Don't Have an Old Database"))
        {
    ...
    {
        //Fixes bug with name enums being created twice (old version and new version of VID)
        updateNamesOnExit = EditorPrefs.GetBool("UpdateNamesOnExit", true);
        EditorPrefs.SetBool("UpdateNamesOnExit", false);
    ...
    {
        File.Delete(currentDir + editorFolderPath + partialClassName + ".cs");  //Delete the no longer needed class
        EditorPrefs.SetBool("UpdateNamesOnExit", updateNamesOnExit);    //Reset to original state

        //Force update name enums
        ItemDatabaseEditorWindow idew = ItemDatabaseEditorWindow.OpenDatabaseWindow();
        idew.UpdateItemNamesEnum(true);
        idew.Close();
